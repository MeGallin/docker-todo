...
async function createApp(database, options = {}) {
  const app = express();
  const todos = await createTodoRepository(database, { encryptionKey: options.encryptionKey });
  ...
  app.get("/api/todos", async (request, response) => {
    ...
    response.json({ todos: await todos.list({ status, priority, search, category, sort, order }) });
  });

  app.get("/api/todos/:id", async (request, response) => {
    const todo = await todos.findById(parseId(request.params.id));
    if (!todo) return response.status(404).json({ error: "Task not found" });
    return response.json({ todo });
  });

  app.post("/api/todos", async (request, response) => {
    const todo = await todos.create(validateTodo(request.body));
  ...
  app.post("/api/todos/:id/toggle", async (request, response) => {
    const id = parseId(request.params.id);
    ...
    return response.json({ todo: await todos.update(id, { status }) });
  });

  app.delete("/api/todos/completed", async (_request, response) => {
    response.json({ deleted: await todos.removeCompleted() });
  });

  app.delete("/api/todos/:id", async (request, response) => {
    const removed = await todos.remove(parseId(request.params.id));
    if (!removed) return response.status(404).json({ error: "Task not found" });
    return response.status(204).end();
  });

  app.get("/api/stats", async (_request, response) => {
    response.json({ stats: await todos.stats() });
  });
